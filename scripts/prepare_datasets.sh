...
python optimize/prepare_webdataset.py | tee "$TMP_LOG"
...
mds_start=$(date +%s)
# Capture output to parse timing information
python optimize/prepare_mosaicdataset.py | tee "$TMP_LOG"
mds_end=$(date +%s)
...
echo "| --- | --- | --- | --- | --- |" >> "$SUMMARY_FILE"
echo "| WebDataset | $webdataset_time_fmt | $webdataset_write_time_fmt | $webdataset_size_gb | $webdataset_files |" >> "$SUMMARY_FILE"
echo "| MDS | $mds_time_fmt | $mds_write_time_fmt | $mds_size_gb | $mds_files |" >> "$SUMMARY_FILE"
# echo "| LitData | $litdata_time_fmt | $litdata_write_time_fmt | $litdata_size_gb | $litdata_files |" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
